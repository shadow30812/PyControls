...
class ModelPredictiveControl:
    ...
    def __init__(
        ...
        self.dt = dt
        self.x_dim = len(x0)
        self.u_dim = 1  # Assuming SISO for this implementation
    ...
    def optimize(self, x_current, x_ref, learning_rate=0.01, iterations=50):
        ...
        for itr in range(iterations):
            ...
            # Compute Gradient w.r.t. each control input u[k]
            # (Simplification: Only compute gradients for first few steps to save time)
            ...
            # if abs(base_cost - new_cost) < 1e-4: break

        # Receding Horizon: Return first control, shift sequence
        u_optimal = self.u_seq[0]

        # Shift: u[0] <- u[1], ..., u[N-1] <- 0
