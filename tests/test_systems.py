...
class TestDCMotor(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.params: Dict[str, Any] = {"J": 0.01, "b": 0.1, "K": 0.01, "R": 1, "L": 0.5}
        self.motor = DCMotor(**self.params)
        self.ss: StateSpace = self.motor.get_state_space()
    ...
    def test_ss_dimensions(self) -> None:
    ...
    def test_param_estimation_dynamics_vectorization(self) -> None:
        f = self.motor.get_parameter_estimation_func()
...
class TestInvertedPendulum(unittest.TestCase):
    ...
    def test_linear_matrix_shapes(self) -> None:
        self.assertEqual(self.pendulum.A.shape, (4, 4))
        self.assertEqual(self.pendulum.B.shape, (4, 1))

    def test_upright_equilibrium_instability(self) -> None:
    ...
    def test_parameter_estimation_shape(self) -> None:
    ...
    def test_parameter_estimation_vectorization(self) -> None:
        f = self.pendulum.get_parameter_estimation_func()
        x = np.ones((6, 10))
        u = np.ones((1, 1))
        dx = f(x, u)
    ...
    def test_measurement_mapping(self) -> None:
        x = np.array([1.0, 2.0, 3.0, 4.0])
        y = self.pendulum.measurement(x)
        np.testing.assert_array_equal(y, [1.0, 3.0])

    def test_measurement_jacobian_constant(self) -> None:
    ...
    def test_mpc_model_callable(self) -> None:
...
class TestLQRLoopTransferFunction(unittest.TestCase):
    ...
    def test_evaluate_finite(self) -> None:
        A = np.array([[0.0]])
        B = np.array([[1.0]])
        K = np.array([[1.0]])
    ...
    def test_evaluate_singularity(self) -> None:
        A = np.array([[1.0]])
        B = np.array([[1.0]])
        K = np.array([[1.0]])
        tf = LQRLoopTransferFunction(A, B, K)
